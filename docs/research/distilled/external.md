---
type: Distilled
title: "Distilled — external evidence (the world's)"
description: "Tier-A facts from the world — mathematics, the gaming ledger, independently replicated findings, official commitments, and an explicit list of what to distrust; external evidence now cited from the lake."
...
[README.md](README.md). Sources: the world's evidence now lives in the evidence lake
(`lake:ai/`, `~/repos/evidence-lake`), cited per row as `lake:ai/<path> §<section> @ <lake-commit>`.
...
rather than on any model or vendor build. **Extended 2026-07-14** by the failure-attribution
pass (`lake:ai/failure-modes/README.md @ 7e3c45f`): five new §3 rows
...
long-running-agents ingestion (`lake:ai/harness-landscape/anthropic-long-running-agents.md @ 7e3c45f`):
...
| **Spec underspecification materially degrades agent success; interactive clarification recovers most of it.** Orchid: pass@1 **−7.22pp avg, up to ~29–32pp**. Ambig-SWE: hiding requirements roughly **halves** resolve on 500 SWE-bench-Verified issues (Fig.-3 band **~40%→~20%**; per-condition digits — including this row's previously-quoted 66%→38% — fail a full text-grep of the paper and are figure-read renderings; corrections ledger 2026-07-14). Specification-Gap: two-agent integration **58%→25%**. Recovery: 80–89% (Ambig-SWE), 71%→81% (ClarifyGPT), +45.73% (TiCoder). Class replicated; magnitudes model-specific. | Orchid (arXiv 2604.21505) + Ambig-SWE (arXiv 2502.13069) + Specification Gap (arXiv 2603.24284) + SWE-Bench Pro on underspecified issues (arXiv 2509.16941: Opus 4.1 22.7→17.8, GPT-5 23.1→14.9); recovery converged by three further mechanisms. *Re-verified 2026-07-14: `lake:ai/failure-modes/root-causes-and-effect-sizes.md §1.4 @ 7e3c45f`* | `llm-class` |
...
| **Weak *benchmark* graders pass semantically-wrong patches at ~8–33%** — distinct mechanism from the row above (suite weakness, not authorship). **19.78%** (2,184/11,041) of top-30 leaderboard "solved" SWE-bench-Verified patches are rejected once tests are adversarially strengthened (independent audit of *others'* patches); independent band **7.8–29.6%**; OpenAI's own ~31% weak-test admission (§2.1). Blindness is necessary, not sufficient: a held-out grader must also be *strong*, and its false-pass rate is the compounding constant of any gated task chain. | SWE-ABS (arXiv 2603.00520) + Wang/Pradel/Liu, ICSE 2026 (arXiv 2503.15223) + OpenAI (§2.1). *Source: `lake:ai/failure-modes/root-causes-and-effect-sizes.md §1–§2 @ 7e3c45f`* | `llm-class` |
...
| **Perceived AI benefit exceeds measured benefit — the overseer's self-assessment channel is miscalibrated in the optimistic direction.** Developers estimated a **+20% speedup** on tasks where the RCT measured them **−19% slower** (16 devs, 246 tasks), and the miscalibration *persisted after* the experience; direction replicated pre-registered at **N=2,691**. The *gap* is the durable finding (METR itself flags the −19% point estimate; §7); it is the measured mechanism behind rubber-stamping an agent's output. | METR RCT (arXiv 2507.09089) + the efficiency-gain-illusion study (arXiv 2605.22687) — independent teams. *Source: `lake:ai/failure-modes/root-causes-and-effect-sizes.md §1, §3 R3 @ 7e3c45f`* | `human-factors` (the AI-benefit setting is `llm-class`) |
...
(`lake:ai/harness-landscape/anthropic-long-running-agents.md §5 @ 7e3c45f`):
...
(`lake:ai/harness-landscape/anthropic-long-running-agents.md §3, §10 @ 7e3c45f`).
