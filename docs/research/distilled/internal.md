---
type: Distilled
title: "Distilled — internal evidence (our own runs)"
description: "Tier-A facts from our own runs — falsified predictions, machinery defects, and wins graded honestly by whether a committed artifact backs them."
...
the `lake:ai/orchestration/concurrency-and-merge-correctness.md @ 7e3c45f`,
...
| **P3v2-5** (pilot-3) — **the design's central thesis, first live proof.** Attempt 1's `get_db(request)` passed visible tests, typecheck, *and* a full 3-lens Opus panel. The **held-out corpus failed 3 tests.** Attempt 2 (fresh worker, no leakage) fixed it and caught a latent `dependency_overrides` closure bug that would have silently broken later DB tasks. | The vault catches what blind validation alone misses | **Committed 2026-07-11** — [`pilot-3-artifacts/`](../internal/pilot-3-artifacts/README.md): the 3 Opus verdicts, the gate stamp, the 11-step gate report, the 7-file held-out corpus, the vault-side `41 passed` log, and the 4-replay meter. Caveat kept: the attempt-1 *failing* log was not retained vault-side — that link survives as the replay timestamps + two-wave run-log, not a preserved failure artifact. | **A3 for the components** (corpus, verdicts, final pass, replay meter); the *comparative* claim — passed-visible-then-failed-held-out — rests on the run-log chain, one preserved artifact short. n=1 |
| **Smoke run 2** (2026-07-12, exec-loop v2) — the thesis's **second live firing**, different failure class: under `bypassPermissions` the implementer could read the held-out workspace (isolation regression); its own visible checks passed, the **held-out test asserting DENIED failed the gate and blocked the merge**. | The held-out suite catches what visible checks pass — including harness misconfiguration, not just code defects | Contemporaneous **committed ledger records** (`v2-ledger.jsonl`, runs 1–2 + fix note), SMOKE.md's run history, and the fix commits. The scratch repo itself was disposable and not preserved. | Committed contemporaneous record of a red run (against interest); the catch itself is existence-grade, n=1 of its class |
...
The 2026-07-11 reincarnation salvage fixed the evidence-practice defect; the component
artifacts grade A3, while the *comparative* half of the claim honestly rests on the run-log
chain (the attempt-1 failing log was never retained). Smoke run 2 (2026-07-12) then fired the
same pattern live a second time — visible checks green, held-out red, merge refused — on a
different failure class (an isolation regression rather than a code defect), recorded in
committed ledger entries though its scratch repo was disposable. Where the thesis stands:
**two live confirmations, neither with a complete preserved failure-side chain, zero
counter-instances — an existence result twice over, still not a catch rate.** T2 remains the
only path to a rate.
