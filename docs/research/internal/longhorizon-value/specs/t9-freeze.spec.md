...
Add the mapping freeze to eaitl as confirm_mapping(ir) -> {version, ir}: the SHA-256 hex digest of the plan's canonical JSON form (sort_keys, compact separators, ensure_ascii=False, UTF-8), refusing malformed plans by raising MappingError when the ratified validator reports any problem. Object key order is identity-irrelevant, array order is identity-relevant, the function is pure and persists nothing. Done when the pinned worked-example digest matches exactly, the canonical-form properties hold, mypy --strict passes, and the existing tests still pass.

## Non-goals

- No storage, registry, or version history — identity only; persistence is the caller's (the pipeline records versions in its own artifacts).
- No signature/attestation of WHO approved — that is the harness's ratification layer, not the product's; this stamps WHAT was approved.
- No canonicalization beyond the pinned rule (no semantic normalization of equivalent plans — e.g. {lit: 100} vs bare 100 freeze differently, as written).
...
- **Problem & who it's for; why now (coverage item 1)**
  Sourced — chain-design task table: the mapping freeze ('confirm_mapping(ir) -> frozen_version: content-addressed, versioned approved IR (deterministic hash)'); next in the ratified cascade order (T2, T3, T4, T5, T6, T7 done).
...
- **Use-cases / consumers (coverage item 3)**
  Sourced — chain-design dependency table: the job pipeline's confirm stage records the version; the CLI reports it; the operator-recorded human-modification channel ends here (edit -> validate -> confirm/freeze).
...
- **Appetite (coverage item 5)**
  Sourced — chain-design size/type table classes this task 'small'; the experiment's measured per-task budget precedent bounds it.
...
- **Canonical form: sorted-keys compact JSON, arrays order-preserving**
  Derived (two-way door, from semantics: key order is presentation, array order is meaning): sort_keys makes cosmetic dict-order differences identity-irrelevant while mapping order (which IS semantics — output key order, last-wins) stays identity-relevant. Alternative (hash the raw file bytes) rejected: whitespace and key order would mint spurious versions. Cost: semantically-equivalent-but-differently-written plans ({lit: 100} vs bare 100) still differ — canonicalizing semantics is out of scope, named.
- **Refuse to freeze malformed plans**
  Derived (two-way door, from approve-before-effect): a frozen version of garbage is worse than no version; composing the ratified validator makes freeze the natural end of the edit -> validate -> confirm channel. Cost: freeze depends on the validator (an intended, recorded composition seam).
- **Identity only, no persistence**
  Derived (two-way door, from one-thing-well): a pure identity function composes anywhere; storage policy belongs to the pipeline task. Cost: callers must record versions themselves.
- **Can a user fix oversights in an approved mapping later, or is the freeze final?**
  Operator question at review (2026-07-16): the freeze is identity, not a lock — edit the plan, re-validate, confirm again; the edit freezes to a new version and the deterministic compilers regenerate code from it (the same loop re-entered, not new product machinery; the job-pipeline task composes it end to end). Cost, named: this slice keeps no version history — current-version bookkeeping is the caller's, and history/diff/rollback are future-product territory.

## Open questions

- none

## Task `mapping-freeze` — Content-address an approved plan with a deterministic version

**Checks:** `python3 -m mypy --strict eaitl` · `python3 -m unittest discover -s tests -t .`
**Provides:** mapping-freeze · **Requires:** eaitl-engine, ir-well-formedness-validator
